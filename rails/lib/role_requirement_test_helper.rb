...
private
  ...
  def request_passes_role_security_system?(action, params)
    ...
    begin
      assert hijacker.hijack_instance_method(action, "@last_action_passed='#{action}'; render :plain => 'passed'"), "unable to hijack method '#{action}'.  Are you sure the action exists?"
      get action, params
    rescue
