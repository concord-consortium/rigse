...
Then /^the portal should create a(?:n)? (.*?) with the following attributes:$/ do |type, table|
  ...
  if (attrs["name"])
    objs = klass.where(name: attrs["name"])
  elsif (attrs["prompt"])
    objs = klass.where(prompt: attrs["prompt"])
  else
    objs = []
  end
...
Then(/^the external activity should not have any description set/) do
  expect(@external_activity.short_description).to be_nil
  expect(@external_activity.long_description).to be_nil
  expect(@external_activity.long_description_for_teacher).to be_nil
end
