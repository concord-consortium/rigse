...
CSV.open("activity_info.csv", "wb") do |csv|
  csv << ["ID", "Parent_ID", "Title", "Link", "Grade Level", "Unit", "Subject Area", "Probes and Models"]
  ...
  #as.each do |a|
    ...
    csv << ([a.id, a.original_id, a.name, "http://itsi.portal.concord.org/activities/#{a.id}.#{a.lightweight? ? 'run_html' : 'jnlp'}", grade_level, unit, subject_area] + pm_list)
  end
end
