...
describe "/maven_jnlp_versioned_jnlps/new.html.erb" do
  include MavenJnlp::VersionedJnlpsHelper
  
  before(:each) do
    assigns[:versioned_jnlp] = stub_model(MavenJnlp::VersionedJnlp,
      :new_record? => true,
  ...
  it "renders new versioned_jnlp form" do
    render
    
    response.should have_tag("form[action=?][method=post]", maven_jnlp_versioned_jnlps_path) do
