...
describe "/maven_jnlp_jars/new.html.erb" do
  include MavenJnlp::JarsHelper
  
  before(:each) do
    assigns[:jar] = stub_model(MavenJnlp::Jar,
      :new_record? => true,
      :uuid => "value for uuid",
      :name => "value for name",
      :main => false,
  ...
  it "renders new jar form" do
    render
    
    response.should have_tag("form[action=?][method=post]", maven_jnlp_jars_path) do
