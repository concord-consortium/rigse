...
describe "/maven_jnlp_maven_jnlp_servers/new.html.erb" do
  include MavenJnlp::MavenJnlpServersHelper
  
  before(:each) do
    assigns[:maven_jnlp_server] = stub_model(MavenJnlp::MavenJnlpServer,
      :new_record? => true,
      :uuid => "value for uuid",
      :host => "value for host",
      :path => "value for path",
  ...
  it "renders new maven_jnlp_server form" do
    render
    
    response.should have_tag("form[action=?][method=post]", maven_jnlp_maven_jnlp_servers_path) do
