...
describe "/help_requests/new.html.erb" do
  include HelpRequestsHelper

  before(:each) do
    assigns[:help_request] = stub_model(HelpRequest,
      :new_record? => true,
      :name => "value for name",
  ...
  it "renders new help_request form" do
    ...
    response.should have_tag("form[action=?][method=post]", help_requests_path) do
