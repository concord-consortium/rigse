...
describe "/otrunk_example_otrunk_view_entries/new.html.erb" do
  include OtrunkExample::OtrunkViewEntriesHelper
  
  before(:each) do
    assigns[:otrunk_view_entry] = stub_model(OtrunkExample::OtrunkViewEntry,
      :new_record? => true,
      :uuid => "value for uuid",
      :otml_import_id => 1,
  ...
  it "renders new otrunk_view_entry form" do
    render
    
    response.should have_tag("form[action=?][method=post]", otrunk_example_otrunk_view_entries_path) do
