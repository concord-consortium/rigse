...
describe API::V1::CreateCollaboration do
  ...
  let(:student1) { FactoryGirl.create(:full_portal_student) }
  let(:student2) { FactoryGirl.create(:full_portal_student) }
  ...
  describe "#call" do
    ...
    context "when offering is an external activity" do
      before do
        offering.runnable = FactoryGirl.create(:external_activity)
        offering.save!
      end
