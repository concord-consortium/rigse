...
def mock_anonymous_user
  if @anonymous_user
    ...
    @anonymous_user.stub!(:has_role?).with("admin", "manager", "teacher", "researcher", "author", "member").and_return(nil)
...
def mock_admin_user
 if @admin_user
   ...
   @admin_user.stub!(:has_role?).with("admin", "manager", "teacher", "researcher", "author", "member").and_return(true)
