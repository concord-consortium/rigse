...
describe RunnablesHelper do
  ...
  describe ".run_link_for" do
    ...
    it "should render a link for an External Activity" do
      ext_act = mock_model(ExternalActivity, :name => "Fetching Wood")
      helper.run_link_for(ext_act).should be_link_like("http://test.host/external_activities/#{ext_act.id}",
                                                       "run_link rollover",
                                                       "/images/run.png")
    end
    ...
    it "should render a link for an Investigation as a JNLP launchable" do
      investigation = mock_model(Investigation, :name => "Searching for Stars")
    ...
    it "should render a link for a Section as a JNLP launchable" do
      section = mock_model(Section, :name => "Learning About Taxidermy")
