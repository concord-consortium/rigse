...
RSpec.feature 'Admin goes to user page', :WebDriver => true do
  let!(:teacher1_user) { FactoryBot.create(:user, login: 'pteacher', password: 'abc123', first_name: 'Pat', last_name: 'Teacher') }
  let!(:teacher2_user) { FactoryBot.create(:user, login: 'rteacher', password: 'abc123', first_name: 'Reilly', last_name: 'Teacher') }
  let!(:student_user) { FactoryBot.create(:user, login: 'cstudent', password: 'abc123', first_name: 'Casey', last_name: 'Student') }
  ...
  let!(:teacher2) { FactoryBot.create(:portal_teacher, user: teacher2_user) }
  ...
  let!(:project_admin_1) { FactoryBot.create(:user, login: 'project_admin_1', password: 'abc123', can_add_teachers_to_cohorts: true) }
  let!(:project_admin_2) { FactoryBot.create(:user, login: 'project_admin_2', password: 'abc123', can_add_teachers_to_cohorts: false) }
  ...
  context "logged in as a project admin that can add teachers to their project's cohorts." do
    before do
      init_project_admin(project_admin_1, project_b)
      @teacher1_name = "#{teacher1_user.first_name} #{teacher1_user.last_name}"
      @teacher2_name = "#{teacher2_user.first_name} #{teacher2_user.last_name}"
      @student_name = "#{student_user.first_name} #{student_user.last_name}"
      @cohort_a_name = "#{project_a.name}: #{cohort_a.name}"
      @cohort_b_name = "#{project_b.name}: #{cohort_b.name}"
      login_as('project_admin_1')
      visit users_path
    end

    scenario 'Project admin can view user page.', js: true do
      expect(current_path).to eq '/users'
      expect(page.body).to match(%r{#{'Show/Hide User Descriptions'}}i)
    end

    scenario 'Project admin cannot see teacher who does not belong to a cohort of their project.', js: true do
      search_for_user(@teacher2_name)
      expect(page).to_not have_content(@teacher2_name)
    end
    ...
    scenario 'Project admin can add a teacher to a cohort and then fully edit user.', js: true do
      ...
      search_for_user(@teacher1_name)
      visit_user_edit_page(@teacher1_name)
      expect(page.body).to match(%r{#{'First Name'}}i)
      expect(page.body).to match(%r{#{'Last Name'}}i)
      expect(page.body).to match(%r{#{'Login'}}i)
      expect(page.body).to match(%r{#{'Email'}}i)
    end
  ...
  context "logged in as a project admin that cannot add teachers to their project's cohorts." do
    before do
      init_project_admin(project_admin_2, project_b)
      @teacher1_name = "#{teacher1_user.first_name} #{teacher1_user.last_name}"
      @student_name = "#{student_user.first_name} #{student_user.last_name}"
      @cohort_name = "#{project_b.name}: #{cohort_b.name}"
      login_as('project_admin_2')
      visit users_path
    end
