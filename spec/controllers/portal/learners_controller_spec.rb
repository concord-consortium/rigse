...
describe Portal::LearnersController do
  ...
  describe "GET config" do
    ...
    xit "should raise an exception when unauthorized config request is made" do
      lambda {
        get :show, :format => :config, :id => @learner.id
      }.should raise_error
    end
  ...
  describe "GET report" do 
    before(:each) do
      ...
      @author_user = Factory.next(:admin_user)
      ...
      @embeddable=Factory.create(:xhtml,:user_id=>@teacher_user.id)
    ...
    it "should show learner report for an activity when filter is set and ignore is set to false for report embeddable filter" do
      ...
      @offering.reload
      assert_equal assigns[:portal_learner].offering.report_embeddable_filter.embeddables, [report_embeddable]
    ...
    it "should show learner report for an activity when filter is set and ignore is set to true for report embeddable filter" do
      ...
      @offering.reload
      assert_equal assigns[:portal_learner].offering.report_embeddable_filter.embeddables, [report_embeddable]
      assert_equal assigns[:portal_learner].offering.report_embeddable_filter.ignore, true
    end
  end
