...
The dashboard app calls three portal endpoints with its launch token, under `/api/v1/researcher_dashboard/`: the class's metadata, a refresh of its authored URL profile, and the run path, which queues packages. The metadata endpoint needs nothing beyond the settings above. The refresh needs only `RESEARCHER_DASHBOARD_FUNCTION_URL`. The run path needs all four, though the CLUE app only for packages that pre-pull CLUE documents. Either answers 503 naming a missing setting it needs:

* `REPORT_SERVER_URL`: report-server's API base URL, where the run path resolves each package in the catalog. Staging is `https://report-server.concordqa.org`; production is the production report-server.
* `RESEARCHER_DASHBOARD_FUNCTION_URL`: report-service's `researcherDashboard` function, which takes `/derive-profile` and `/run-package`. Staging is `https://us-central1-report-service-dev.cloudfunctions.net/researcherDashboard`; production is the same function in `report-service-pro`.
* `RESEARCHER_DASHBOARD_FIREBASE_APP`: the FirebaseApp for report-service's Firebase project (`report-service-dev` on staging, `report-service-pro` in production). The runner's session and class tokens are minted with it, and its name, which is the project id, is sent as the runner's `firebase_project`.
* `RESEARCHER_DASHBOARD_CLUE_FIREBASE_APP`: the FirebaseApp for CLUE's Firebase project (`collaborative-learning-staging` on staging, CLUE's production project in production), used only for packages that pre-pull CLUE documents. It goes away once the runner reads CLUE through cc-data (RIGSE-369).

Both FirebaseApp names must be rows in the portal's FirebaseApps admin page, which `jwt/firebase` already needs for the app's own tokens, or the run path answers 503. `REPORT_SERVICE_URL` and `REPORT_SERVICE_BEARER_TOKEN` are unrelated to the dashboard and stay for feedback metadata.
...
On AWS these are the `ResearcherDashboardUrl`, `PortalSigningKey`, `PortalSigningKeyId`, `PortalPreviousVerifyKeys`, `ReportServerURL`, `ResearcherDashboardFunctionURL`, `ResearcherDashboardFirebaseApp` and `ResearcherDashboardClueFirebaseApp` stack parameters, all empty by default, and an empty one is left out of the task environment. A normal release updates the stack with its previous template, so releasing this code does not add them to a running stack and the dashboard stays disabled there. Enabling it is a deliberate template update per environment with that environment's own values, after which releases carry them forward.
