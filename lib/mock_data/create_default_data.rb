module MockData
    ...
    DEFAULT_DATA[:teacher_clazzes].each do |teacher_clazz, teacher_clazz_info|
      ...
      if map_teacher
        ...
        clazz_names.each do |clazz_name|
          ...
          if map_clazz
            teacher_clazz = Portal::TeacherClazz.xfind_or_create_by_teacher_id_and_clazz_id(map_teacher.id, map_clazz.id)
          end
        end
      end
    end
    ...
    DEFAULT_DATA[:student_clazzes].each do |student_clazz, student_clazz_info|
      ...
      if map_student
        ...
        clazz_names.each do |clazz_name|
          map_clazz = default_classes.find{|c| c.name == clazz_name}
          if map_clazz
            student_clazz = Portal::StudentClazz.xfind_or_create_by_student_id_and_clazz_id(map_student.id, map_clazz.id)
          end
        end
      end
    end
  ...
  def self.add_multichoice_answer(learner,question,answer_text, data)
    ...
    if new_answer_by_uuid
      ...
      saveable_answer = Saveable::MultipleChoiceAnswer.xfind_or_create_by_uuid(data[:saveable_multiple_choice_answers_uuid])
      saveable_answer.multiple_choice = new_answer
      saveable_answer.save!

      saveable_mc_rationale_choice = Saveable::MultipleChoiceRationaleChoice.xfind_or_create_by_uuid(data[:saveable_multiple_choice_rationale_choices_uuid])
  ...
  def self.add_image_question_answer(learner,question,answer_text,data)
    ...
    if new_answer_by_uuid
      ...
      blob = Dataservice::Blob.xfind_or_create_by_uuid[data[:dataservice_blob_uuid]]
      blob.content = answer_text
      blob.token = answer_text
      blob.save!

      saveable_answer = Saveable::ImageQuestionAnswer.xfind_or_create_by_uuid(data[:saveable_image_question_answer_uuid])
