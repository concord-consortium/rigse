...
export default class ClassSetupForm extends React.Component {
  constructor (props) {
    super(props)
  ...
  handleSetCurrentTeachers (currentTeachers) {
  ...
  render () {
    ...
    activeGrades.forEach(function (activeGrade, index) {
    ...
    return (
      <>
                ...
                ? <tr>
                  ...
                  <td>
                    <table style={{ width: '100%' }}>
                      <tbody>
                        {gradeLevelRows.map(function (row, rowIndex) {
                          return (
                            <tr key={rowIndex}>
                              {row.map(function (grade) {
        ...
        {updating ? <input type='hidden' name={currentTeachersField.name} value={currentTeachers.map(t => t.id).join(',')} /> : undefined}
      </>
    )
  }
}
