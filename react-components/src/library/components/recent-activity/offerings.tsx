...
export default class Offerings extends React.Component<any, any> {
  render () {
    ...
    if (!anyClasses) {
      return (
        <>
          <p>To get started, add a new class by clicking "Classes" on the left and then "Add Class." Enter the class setup information, including class name, description, applicable grade level(s), and specify a unique class word. Students will use the unique class word to enroll in the class.</p>
    ...
    if (!anyData) {
      return (
        <>
          ...
          <p><strong>You need to assign at least one activity to your classes.</strong> To add an assignment, click "Classes" on the left, then on the name of a class. Next click the "Assignments" link for that class, and then the "Find More Resources" button to view assignable activities.</p>
