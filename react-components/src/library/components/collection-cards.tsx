...
const CollectionCards = Component({
  ...
  componentDidMount () {
    waitForAutoShowingLightboxToClose(function () {
      jQuery.ajax({
        ...
        let collections = data.reduce(function (_collections: any, collection: any) {
  ...
  renderCollectionCards () {
    const collectionsCards: any = [];
    const defaultProjectCardImageUrl = "https://learn-resources.concord.org/images/collections/default-collection.jpg";
    this.state.collections.map(function (collection: any) {
      collectionsCards.push(
        <div key={collection.landing_page_slug} className={"portal-pages-collections-card col-4"}>
