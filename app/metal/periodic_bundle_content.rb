...
# /dataservice/periodic_bundle_loggers/7/periodic_bundle_contents.bundle

class PeriodicBundleContent
  ...
  def self.call(env)
    ...
    bundle_content = nil
    bundle_logger_id = env[PATH_INFO][/\/dataservice\/periodic_bundle_loggers\/(\d+)\/periodic_bundle_contents\.bundle/, 1]
    if env[REQUEST_METHOD] == POST && bundle_logger_id && bundle_logger = ::Dataservice::PeriodicBundleLogger.find(bundle_logger_id)
