...
xml.jnlp(:spec => "1.0+", :codebase => "http://#{current_project.jnlp_cdn_hostname.presence || 'jnlp.concord.org'}/dev3") { 
  ...
  xml.resources {
    ...
    xml.jar :href=> "org/concord/utilities/response-cache/response-cache-0.1.0-20140103.181809-221.jar"
    xml.jar :href=> "org/concord/jnlp2shell/jnlp2shell-1.0-20140103.235516-465.jar", :main =>"true"
    ...
    if(opportunistic_installer)
      ...
      xml.property :name=> "jnlp.wrapped_jnlp", :value => jnlp_adaptor.jnlp_url
  ...
  if(!opportunistic_installer)
    xml.resources(:os => "Linux") { 
      xml.property :name=> "jnlp.not_found_url", :value => jnlp_installer_not_found_url("linux")
    }
    xml.resources(:os => "Mac OS X") { 
      xml.property :name=> "jnlp.not_found_url", :value => jnlp_installer_not_found_url("osx")
    }
    xml.resources(:os => "Windows") { 
      xml.property :name=> "jnlp.not_found_url", :value => jnlp_installer_not_found_url("windows")
    }      
  end
