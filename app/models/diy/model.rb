    ...
    def from_external_portal(_diy_model)
      ...
      if ITSI_ASSET_URL
        ...
        if attributes["url"] && attributes["url"].size > 0
          attributes["url"] = ITSI_ASSET_URL.merge(URI.escape(attributes["url"].strip)).to_s
        end

        ## also check the image url
        if attributes["image_url"] && attributes["image_url"].size > 0
          attributes["image_url"] = ITSI_ASSET_URL.merge(URI.escape(attributes["image_url"].strip)).to_s
        end
      end
