class Search
  ...
  def initialize(opts={})
    ...
    self.include_archived     = opts[:include_archived]    || false
  ...
  def search
    ...
    self.clean_material_types.each do |type|
      _results = self.engine.search(SearchableModels) do |s|
        ...
        s.without(:is_archived, true) unless self.include_archived
        s.with(:is_archived, true) if self.include_archived
