class BigIdeasController < ApplicationController
  # GET /big_ideas
  # GET /big_ideas.xml
  def index
    @big_ideas = BigIdea.find(:all)

    respond_to do |format|
      format.html # index.html.erb
      format.xml  { render :xml => @big_ideas }
    end
  end

  # GET /big_ideas/1
  # GET /big_ideas/1.xml
  def show
    @big_idea = BigIdea.find(params[:id])

    respond_to do |format|
      format.html # show.html.erb
      format.xml  { render :xml => @big_idea }
    end
  end

  # GET /big_ideas/new
  # GET /big_ideas/new.xml
  def new
    @big_idea = BigIdea.new

    respond_to do |format|
      format.html # new.html.erb
      format.xml  { render :xml => @big_idea }
    end
  end

  # GET /big_ideas/1/edit
  def edit
    @big_idea = BigIdea.find(params[:id])
  end

  # POST /big_ideas
  # POST /big_ideas.xml
  def create
    @big_idea = BigIdea.new(params[:big_idea])

    respond_to do |format|
      if @big_idea.save
        flash[:notice] = 'BigIdea was successfully created.'
        format.html { redirect_to(@big_idea) }
        format.xml  { render :xml => @big_idea, :status => :created, :location => @big_idea }
      else
        format.html { render :action => "new" }
        format.xml  { render :xml => @big_idea.errors, :status => :unprocessable_entity }
      end
    end
  end

  # PUT /big_ideas/1
  # PUT /big_ideas/1.xml
  def update
    @big_idea = BigIdea.find(params[:id])

    respond_to do |format|
      if @big_idea.update_attributes(params[:big_idea])
        flash[:notice] = 'BigIdea was successfully updated.'
        format.html { redirect_to(@big_idea) }
        format.xml  { head :ok }
      else
        format.html { render :action => "edit" }
        format.xml  { render :xml => @big_idea.errors, :status => :unprocessable_entity }
      end
    end
  end

  # DELETE /big_ideas/1
  # DELETE /big_ideas/1.xml
  def destroy
    @big_idea = BigIdea.find(params[:id])
    @big_idea.destroy

    respond_to do |format|
      format.html { redirect_to(big_ideas_url) }
      format.xml  { head :ok }
    end
  end
end
