  ...
  def show
    ...
    if request.xhr?
      ...
      respond_to do |format|
        ...
        format.jnlp   { render :partial => 'shared/installer', :locals => { :runnable => @video_player } }

        format.config { render :partial => 'shared/show', :locals => { :runnable => @video_player, :session_id => (params[:session] || request.env["rack.session.options"][:id]) } }
        format.dynamic_otml { render :partial => 'shared/show', :locals => {:runnable => @video_player } }
