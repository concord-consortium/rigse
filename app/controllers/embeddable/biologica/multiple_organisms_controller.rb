  ...
  def show
    ...
    if request.xhr?
      ...
      respond_to do |format|
        ...
        format.jnlp { render :partial => 'shared/show_or_installer', :locals => { :runnable => @biologica_multiple_organism, :teacher_mode => false  }}
        format.config { render :partial => 'shared/show', :locals => { :runnable => @biologica_multiple_organism, :session_id => (params[:session] || request.env["rack.session.options"][:id]), :teacher_mode => false  } }
