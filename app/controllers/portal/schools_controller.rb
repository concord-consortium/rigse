class Portal::SchoolsController < ApplicationController
  ...
  def create
    ...
    change_use_installer = (params[:settings] && params[:settings][:use_installer])
    use_installer = (params[:settings][:use_installer] == "1") if change_use_installer
    ...
    if request.xhr?
      if cancel 
        ...
        @portal_school.put_setting("use_installer", "1") if use_installer && change_use_installer
  ...
  def update
    ...
    if request.xhr?
      if cancel || @portal_school.update_attributes(params[:portal_school])
        unless cancel || !change_use_installer
          if use_installer
