...
var EnableWorkgroup = function(el,waiting_callback) {
    var offering_id = ParseOfferingUrl(el.href);
    // remove other click observers!
    el.stopObserving('click');
    el.observe('click', function(e) {
      Workgroup(offering_id,el.href,waiting_callback);
      e.preventDefault();
      // e.stop();
    });
};

var EnableWorkgroups = function(_selector,waiting_callback) {
  _selector = _selector ||   'a.run_link';
  waiting_callback = waiting_callback || showWait; // showWait used to be global.
  $$(_selector).each(function(el) {
    EnableWorkgroup(el,waiting_callback);
  });
};

var Workgroup = function(_offering,_launch_url,_waiting_callback) {
  ...
  var launch_action = function() {
    ...
    lightbox_hood.hide();
    lightbox_content.hide();
    waiting_callback(offering);
    // Make this request synchronously, so we can be sure it finishes before
    // the normal link click handling happens.
    new Ajax.Request(API_V1.COLLABORATIONS, {
      method: 'post',
      contentType: 'application/json',
      parameters: JSON.stringify({
        offering_id: offering,
        students: collaborators.map(function (l) { return {id: l.id, password: l.password}; })
      }),
      asynchronous: false,
      onSuccess: function() {
        pending_requests = pending_requests -1;
  ...
  var check_password = function(learner, passwd, succ, fail) {
    pending_requests = pending_requests + 1;
    new Ajax.Request(API_V1.STUDENT_CHECK_PASSWORD.replace(/999/, learner.id), {
      method: 'post',
      contentType: 'application/json',
      parameters: JSON.stringify({'password': passwd}),
      onSuccess: function(transport) {
        pending_requests = pending_requests - 1;
        learner.password = passwd;
        succ.call();
      },
      onFailure: function() {
        pending_requests = pending_requests - 1;
