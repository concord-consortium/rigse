...
**What it builds on (RIGSE-367, `specs/RIGSE-367-the-portal-signing-key-and-the-scoped-launch-token/`).** None of RIGSE-367 is implemented yet; its branch holds only its spec, so every name below arrives with that story.
