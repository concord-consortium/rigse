...
It might make good sense to use "RVM":http://rvm.beginrescueend.com/ to isolate gem sets
while you are working on this project. Here is one pattern you could use;
...
Next time you work on (eg has) try:

<pre><code>rvm use 1.8.7@has</pre></code>
...
<pre><code>
  ...
  # if you are going to make your own theme, copy a sample first
  # cp ./config/themes/has/settings.sample.yml ./config/themes/xproject
  # cp -r ./themes/has ./themes/xproject
  # cp -r ./public/stylesheets/themes/has ./public/stylesheets/themes/xproject
  # cp -r ./public/stylesheets/sass/themes/has ./public/stylesheets/sass/themes/xproject
  ...
  ruby -S rake gems:install
  RAILS_ENV=cucumber ruby -S rake gems:install

  # setup database
  RAILS_ENV=production ruby rake db:migrate:reset

  # setup application resources
  RAILS_ENV=production ruby rake rigse:setup:new_rites_app

  # save a copy of the development database to make subsequent clean starts much quicker
  # Saves database to: db/development_data.sql
  # can be reloaded with: rake db:load
  ruby rake db:dump
...
<pre><code>  cap production db:fetch_remote_db</code></pre>
...
<pre><code>  cap staging db:push_remote_db</code></pre>
