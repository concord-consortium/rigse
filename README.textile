...
@LD_LIBRARY_PATH=/opt/local/lib@ to have nokogiri check there for shared libraries first.
...
bq. @LD_LIBRARY_PATH=/opt/local/lib jruby -S rake spec @ _options_

*Running all the rspec tests:*
bc.rake spec

*Running a single file:*
bc. rake spec SPEC=spec/routing/dataservice/bundle_contents_routing_spec.rb
...
*JRuby invocation note*: use this command prefix to run the rake spec tests from JRuby if you have a more recent version of libxml2 installed with macports: @LD_LIBRARY_PATH=/opt/local/lib jruby -S@
...
bc. rake cucumber FEATURE=features/student_can_not_see_deactivated_offerings.feature
...
The rake tasks assume that bitrock is in the standard /Applications/ folder.
You can override this by setting an environment variable in your shell which points to the correct path, eg: @export BITROCK_INSTALLER=/path/to/bitrock.app@
...
The jnlp_url should point to a jnlp url on the target host.  When you run the
