...
After checking out the code for the first time switch to the np2 branch:

  git branch --track np2 origin/np2
  git checkout -b np2


cp config/database.sample.yml config/database.yml

rake rigse:setup:regenerate_rest_auth_site_key
...
h2. Capistrano Recipes (and problems)

See config/deploy.rb for intimate details.

<code>cap deploy</code> should work to deploy to development (rites.dev.concord.org (otto)).
<code>cap staging deploy</code> will go to staging (rites-investigations.staging.concord.org (seymour)).
<code>cap production deploy</code> will go to production (rites-investigations.concord.org (seymour)).
