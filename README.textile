...
Working git, ruby or jruby, and rubygems, wget
...
<pre><code>
  ...
  # setup rvm with a gemset named xproject
  rvm use 1.8.7
  rvm gemset create xproject

  # make .rvmrc so rvm will use this ruby gemset combo automatically
  echo "rvm use 1.8.7@xproject" > .rvmrc

  # test out .rvmrc and start using the gemset
  cd ..;cd xproject
  ...
  # automatic setup of config/settings.yml and config/database.yml
  ruby config/setup.rb -n "Cross Project Portal" -D xproject -u <dbuser> -p <password> -t xproject -y -q -f

  # The options -y -q -f to setup mean: 
  #   answer 'yes' to accept default values
  #   use 'quieter' console output
  #   force updates of the existing settings.yml and databse.yml if they exist

  # setup database
  # if this fails it is probably due to a bad mysql gem install, check out this page for help
  #  https://rvm.beginrescueend.com/integration/databases/
  RAILS_ENV=production rake db:create
  RAILS_ENV=production rake db:migrate:reset

  # setup application resources
  # this can take 5 minutes or so and requires answering some questions  
...
h3. Setting up administrative users

from the command line:
<pre><code>
script/console
username = User.find_by_login('username')
...
<pre><code>  cap <stage> db:copy_remote_site_keys</code></pre>
